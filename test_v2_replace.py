"""Document replacement: [sys+doc1+doc2] stored -> [sys+doc1+doc3] used.
doc3 is completely different from doc2. This should create real cache mismatch."""
import torch
...
store = ChunkStore("./chunk_store_replace")
...
# doc3: completely different topic (history of computing)
...
# Queries that span doc1 and doc3 (NOT doc2)
queries_doc1 = [
...
stored_context = doc1 + " " + doc2   # [doc1 + doc2]
new_context = doc1 + " " + doc3      # [doc1 + doc3]
...
for ratio in [0.15, 0.30, 0.50]:
    ...
    store.save_chunk(f"replace_{ratio}", kv)
    surviving = sum(kv.info["len_k"][0]).item()
    total = kv._seen_tokens
    print(f"Stored [doc1+doc2]: {total} tokens, surviving: {surviving} ({surviving/total*100:.0f}%)")

    # Baseline: full prefill [doc1+doc3]
    ...
    for q, kw in queries_doc3:
        ...
        if kw.lower() in out.lower():
            bl_d3 += 1
    print(f"Baseline [doc1+doc3]: doc1={bl_d1}/{len(queries_doc1)} doc3={bl_d3}/{len(queries_doc3)} total={bl_d1+bl_d3}/{len(queries)}")

    # No blend: use stored [doc1+doc2] KV for [doc1+doc3] queries
    kv_no = store.load_chunk(f"replace_{ratio}", device=model.device)
    no_d1 = no_d3 = 0
    for q, kw in queries_doc1:
        qi = model.apply_template(q + "\nAnswer briefly with the exact fact.")
    ...
    for q, kw in queries_doc3:
        ...
        if kw.lower() in out.lower():
            no_d3 += 1
    print(f"No blend:            doc1={no_d1}/{len(queries_doc1)} doc3={no_d3}/{len(queries_doc3)} total={no_d1+no_d3}/{len(queries)}")

    # Blend with new context [doc1+doc3]
    ...
    for method in methods:
        for r in recomp_ratios:
            m_d1 = m_d3 = 0
            for q, kw in queries_doc1:
                kv_bl = store.load_chunk(f"replace_{ratio}", device=model.device)
                qi = model.apply_template(q + "\nAnswer briefly with the exact fact.")
                out = model.blend_generate_v2(
                    qi, [kv_bl], recomp_ratio=r, check_layers=[1],
                    method=method, context=new_context)
                if kw.lower() in out.lower():
                    m_d1 += 1
            for q, kw in queries_doc3:
                kv_bl = store.load_chunk(f"replace_{ratio}", device=model.device)
                ...
                if kw.lower() in out.lower():
                    m_d3 += 1
            print(f"  {method:>16s} r={r:.2f}  doc1={m_d1}/{len(queries_doc1)} doc3={m_d3}/{len(queries_doc3)} total={m_d1+m_d3}/{len(queries)}")
