"""Document reorder test v2 - pass reordered context to blend_generate_v2."""
import torch
...
store = ChunkStore("./chunk_store_reorder_v2b")
...
for ratio in [0.15, 0.30]:
    ...
    # No blend (use stored [A+B] KV directly)
    ...
    for q, kw in queries:
        ...
        if kw.lower() in out.lower():
            no_matches += 1
    print(f"No blend (stored [A+B] order): {no_matches}/{len(queries)}")

    # Blend with reordered context [B+A]
    ...
    for method in methods:
        for r in recomp_ratios:
            ...
            for q, kw in queries:
                ...
                qi = model.apply_template(q + "\nAnswer briefly.")
                # Pass combined_BA as the NEW context
